...
class KVPoolWorker:
    ...
    def _validate_put_cache_blocks(
        self,
        group_id: int,
        block_ids: list[int],
        request_id: str,
        ...
        """Reject non-finite cache blocks immediately before an eager P-side put."""
        if self.kv_cache_config is not None and self.use_hybrid:
            ...
            if isinstance(kv_cache_spec, UniformTypeKVCacheSpecs):
                ...
                specs = [kv_cache_spec]
            # DeepSeek-V4 compressor state groups (for example C4A/C128A)
            # have different valid regions from token KV/SWA blocks. Exclude
            # them from this diagnostic; regular cache groups remain checked.
            if any(getattr(spec, "state_compress_ratio", 1) > 1 for spec in specs):
                return
        ...
        if self.kv_cache_config is not None and self.use_hybrid:
            ...
            layer_names = list(self.kv_caches)

        finite_checks: list[torch.Tensor] = []
        for block_id in unique_block_ids:
            if block_id < 0 or block_id >= self.num_blocks:
                raise RuntimeError(
                    "Invalid KV cache block ID before Store put: "
                    f"request_id={request_id}, group_id={group_id}, "
                    f"block_id={block_id}, num_blocks={self.num_blocks}, keys={keys}."
                )
            for layer_name in layer_names:
                cache_or_caches = self.kv_caches[layer_name]
                for cache in self._as_cache_tuple(cache_or_caches):
                    _, _, _, block_size_scale = self._get_cache_block_metadata(cache)
                    block_start = block_id * block_size_scale
                    block_end = block_start + block_size_scale
                    finite_checks.append(
                        torch.isfinite(cache[block_start:block_end]).all()
                    )

        # Cache data remains on NPU. Only this combined boolean is materialized
        # by Python so a RuntimeError can prevent the Store put.
        if finite_checks and not bool(torch.stack(finite_checks).all()):
            raise RuntimeError(
                "Non-finite value (NaN or Inf) found in KV cache before Store put: "
                f"request_id={request_id}, group_id={group_id}, "
                f"block_ids={unique_block_ids}, keys={keys}."
            )
        ...
        if self.use_layerwise:
            ...
            if self.kv_role in ["kv_producer", "kv_both"] or self.consumer_is_to_put:
                ...
                self.kv_send_thread = KVCacheStoreSendingThread(
                    ...
                    self._validate_put_cache_blocks
                    if self.validate_put_cache_blocks
                    else None,
                )
                self.kv_send_thread.start()
            if self.load_async:
    ...
    def wait_for_save(self, connector_metadata: AscendConnectorMetadata):
        ...
        if has_save_request:
            ...
            if isinstance(self.kv_send_thread, KVCacheStoreSendingThread):
                self.kv_send_thread.raise_put_error()
