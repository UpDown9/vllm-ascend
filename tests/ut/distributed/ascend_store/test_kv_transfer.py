...
class TestKVCacheStoreSendingThread(unittest.TestCase):
    ...
    def test_state_cache_group_uses_tp_interleaved_put(self):
        ...
        req = ReqMeta(
            ...
            block_hashes=[b"h0", b"h1", b"h2", b"h3"],  # type: ignore[arg-type]
            current_event=None,
            dsa_cp_swa_owner_ranges=[(256, 512)],
        )
    ...
    def test_filter_dsa_cp_swa_owner_blocks_assigns_crossing_block_to_end_owner(self):
        t = KVCacheStoreSendingThread.__new__(KVCacheStoreSendingThread)
        t.group_uses_swa = [True]
        t.cp_rank = 1
        t.cp_size = 2
        ...
        self.assertEqual(starts, [192])
        self.assertEqual(ends, [320])
        self.assertEqual(keys, ["k_partial"])
        self.assertEqual(hashes, ["h_partial"])
    ...
    def test_pool_worker_assigns_local_cache_swa_owner_after_flatten_padding(self):
        expected_ranges = [
            ([(0, 128)], []),
            ([(128, 200)], []),
            ([], [(0, 128)]),
        ...
        for tp_rank, (expected_req0, expected_req1) in enumerate(expected_ranges):
            worker = KVPoolWorker.__new__(KVPoolWorker)
            worker.tp_size = 4
            ...
            req0 = ReqMeta(
                ...
                can_save=True,
                save_end_token=200,
                target_token_len=200,
                current_chunk_end_token=200,
            )
            req1 = ReqMeta(
                req_id="r1",
                token_len_chunk=200,
            ...
            worker._assign_dsa_cp_swa_owner_ranges_for_save_batch(
                [req0, req1],
                [("r0", 200), ("r1", 200)],
            )

            self.assertEqual(req0.dsa_cp_swa_owner_ranges, expected_req0)
            self.assertEqual(req1.dsa_cp_swa_owner_ranges, expected_req1)

    def test_local_cache_swa_owner_uses_full_chunked_prefill_batch_layout(self):
        expected_ranges = [[], [(1024, 1128)], [(1128, 1180)]]

        for tp_rank, expected in enumerate(expected_ranges):
            worker = KVPoolWorker.__new__(KVPoolWorker)
            worker.tp_size = 3
            worker.tp_rank = tp_rank
            worker.group_uses_swa = [True]
            ...
            saved_req = ReqMeta(
                req_id="saved",
                token_len_chunk=1180,
                can_save=True,
                save_start_token=1024,
                save_end_token=1180,
                target_token_len=1200,
                current_chunk_start_token=1000,
                current_chunk_end_token=1200,
            )

            worker._assign_dsa_cp_swa_owner_ranges_for_save_batch(
                [saved_req],
                [("not_saved", 64), ("saved", 200)],
            )

            self.assertEqual(saved_req.dsa_cp_swa_owner_ranges, expected)

    def test_local_cache_swa_owner_requires_model_batch_layout(self):
        worker = KVPoolWorker.__new__(KVPoolWorker)
        worker.group_uses_swa = [True]
        worker.use_dsa_cp_local_cache = True
        request = ReqMeta(req_id="r0", token_len_chunk=128, can_save=True)

        with self.assertRaisesRegex(RuntimeError, "requires the model batch layout"):
            worker._assign_dsa_cp_swa_owner_ranges_for_save_batch([request])
