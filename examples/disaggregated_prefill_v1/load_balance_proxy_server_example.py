...
def _is_context_length_error(exc: Exception) -> bool:
    """Identify decoder validation errors caused by model context limits."""
    response = getattr(exc, "response", None)
    if getattr(response, "status_code", None) not in (400, 422):
        return False
    message = _get_exception_message(exc).lower()
    return any(
        phrase in message
        for phrase in (
        ...
        async def generate_stream():
            ...
            try:
                ...
                if _is_context_length_error(exc):
                    ...
                    logger.info(
                        "Decoder rejected request due to context length: request_id=%s",
                        instance_info.request_id,
                    )
                logger.error(
